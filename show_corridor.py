...
from utils import save_plot, ensure_dir, softmax
import random

np.random.seed(0)
random.seed(0)

env_name = 'ShortCorridor-v0'
...
print("Policy for right action: {}".format(agent._policy(np.array([0,1]))[1]))
# plotting
launch_specs = 'baseline'+version+'lr.001'
...
save_plot(rewards_history, file_name, suptitle, title, xlabel, ylabel,
          smooth_avg=n_episodes//50, only_avg=True)
