...
class Sarsa(Agent):
    ...
    def act(self, x):
        ...
        if np.random.rand() < self.epsilon:
            ...
            action_values = self.w.T@x # definition of linear q_hat
    ...
    def learn(self, x, a, r, x_, d=None):
        ...
        delta = r + self.gamma*(self.w.T@x_)[self.act(x_)] - (self.w.T@x)[a]
        grad = np.zeros_like(self.w)
        grad[:,a] = x
        self.w += self.learn_rate*delta*grad
        # anneal epsilon
        self.step += 1
...
class QLearning(Sarsa):
    ...
    def learn(self, x, a, r, x_, d=None):
        ...
        Updates the annealing epsilon. """
        delta = r + self.gamma*max(self.w.T@x_) - (self.w.T@x)[a]
        grad = np.zeros_like(self.w)
        grad[:,a] = x
        self.w += self.learn_rate*delta*grad
        self.step += 1
